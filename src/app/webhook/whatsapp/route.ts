...
export async function GET(request: Request) {
  ...
  console.log(`Webhook GET params - mode: ${mode}, token: ${token}, challenge: ${challenge}, expected: ${verifyToken}`);
  ...
  console.warn(`WhatsApp Webhook verification failed: token mismatch or invalid mode. received: ${token}, expected: ${verifyToken}`);
...
export async function POST(request: Request) {
  try {
    ...
    if (isSandboxSim) {
      ...
      rawMessageText = (body.messageText || "").trim();
      messageText = rawMessageText.toUpperCase();
      console.log(`[SANDBOX] Incoming WhatsApp Message - Sender: ${senderPhone}, Message: "${rawMessageText}"`);
    } else {
      ...
      rawMessageText = (message.text?.body || "").trim();
      messageText = rawMessageText.toUpperCase();
      console.log(`Incoming WhatsApp Message - Sender: ${senderPhone}, Message: "${rawMessageText}"`);
    }

...
export default function CheckInPage() {
  ...
  // 1.2 Dynamically load face-api.js script from CDN
  useEffect(() => {
  ...
  const submitCheckIn = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setSuccessData({
          success: false,
          error: data.error || "Failed to log check-in",
        });
      } else {
        setSuccessData({
          success: true,
        ...
        <div className={`${styles.card} glass-panel`}>
          ...
          <div className={styles.header}>
            <h1 className={styles.title}>
              {company ? `${company.name} Check-In` : "Check-In Portal"}
            </h1>
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={`${styles.card} glass-panel`}>
        ...
        <div className={styles.header}>
          <h1 className={styles.title}>
            {company ? `Mark Check-In (${company.name})` : "Mark Check-In"}
          </h1>
